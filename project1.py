...
_resp_cache:  dict[str, str] = {}  # кеш всех ответов

ALL_RESPONSES = {
    # Громкость
    "volume_up_10":   "Громкость 10%.",
    "volume_up_15":   "Громкость 15%.",
    "volume_up_20":   "Громкость 20%.",
    "volume_up_25":   "Громкость 25%.",
    "volume_up_30":   "Громкость 30%.",
    "volume_up_35":   "Громкость 35%.",
    "volume_up_40":   "Громкость 40%.",
    "volume_up_45":   "Громкость 45%.",
    "volume_up_50":   "Громкость 50%.",
    "volume_up_55":   "Громкость 55%.",
    "volume_up_60":   "Громкость 60%.",
    "volume_up_65":   "Громкость 65%.",
    "volume_up_70":   "Громкость 70%.",
    "volume_up_75":   "Громкость 75%.",
    "volume_up_80":   "Громкость 80%.",
    "volume_up_85":   "Громкость 85%.",
    "volume_up_90":   "Громкость 90%.",
    "volume_up_95":   "Громкость 95%.",
    "volume_up_100":  "Громкость 100%.",
    "volume_max":     "Громкость максимальная.",
    "volume_min":     "Громкость минимальная.",
    "sound_off":      "Звук выключен.",
    "sound_on":       "Звук включён.",
    "volume_up_fallback":   "Громкость увеличена.",
    "volume_down_fallback": "Громкость уменьшена.",
    # Яркость
    "brightness_up":    "Яркость увеличена.",
    "brightness_down":  "Яркость уменьшена.",
    "brightness_20":    "Яркость 20%.",
    "brightness_40":    "Яркость 40%.",
    "brightness_60":    "Яркость 60%.",
    "brightness_80":    "Яркость 80%.",
    "brightness_100":   "Яркость 100%.",
    # WiFi
    "wifi_on":   "WiFi включён.",
    "wifi_off":  "WiFi выключен.",
    # Окна
    "window_minimize":  "Окна свёрнуты.",
    "window_maximize":  "Окно развёрнуто.",
    "window_close":     "Окно закрыто.",
    "switch_window":    "Переключаю.",
    # Буфер
    "clipboard_copy":   "Скопировано.",
    "clipboard_paste":  "Вставлено.",
    "clipboard_empty":  "Буфер пуст.",
    # Скриншот
    ...
    # Приложения
    "app_telegram":     "Телеграм открыт.",
    "app_discord":      "Дискорд открыт.",
    "app_spotify":      "Спотифай открыт.",
    "app_chrome":       "Хром открыт.",
    "app_notepad":      "Блокнот открыт.",
    "app_calculator":   "Калькулятор открыт.",
    "app_explorer":     "Проводник открыт.",
    "app_steam":        "Стим открыт.",
    "app_obs":          "OBS открыт.",
    "app_vscode":       "VS Code открыт.",
    "app_word":         "Word открыт.",
    "app_excel":        "Excel открыт.",
    # Пинг
    "ping_0":  "Да?",
    "ping_1":  "Я здесь.",
    "ping_2":  "Слушаю.",
    # Размут
    "unmute_0": "Снова слушаю.",
    "unmute_1": "Да?",
    "unmute_2": "Я здесь.",
    # Режимы
    "mode_night":        "Ночной режим. Яркость снижена, уведомления отключены.",
    "mode_presentation": "Режим презентации. PowerPoint, Телеграм, загрузки и Gamma открыты.",
    # Папки
    "folder_downloads":  "Загрузки открыта.",
    "folder_documents":  "Документы открыта.",
    "folder_desktop":    "Рабочий стол открыт.",
    "folder_music":      "Музыка открыта.",
    "folder_video":      "Видео открыто.",
    "folder_images":     "Изображения открыты.",
    # Прочее
    "dictated":          "Напечатала.",
    "battery_missing":   "Батарея не найдена.",
    "weather_no_key":    "Добавь WEATHER_API_KEY в .env.",
...
def speak_resp(key: str) -> bool:
    """Мгновенное воспроизведение кешированного ответа. Возвращает True если нашёл."""
...
def volume_up() -> str:
    ...
    if v:
        ...
        # округляем до ближайших 5% для кеша
        pct5 = round(pct / 5) * 5
        key = f"volume_up_{pct5}"
        if not speak_resp(key):
            speak(f"Громкость {pct}%.")
        return ""
    try:
        import pyautogui
        for _ in range(5): pyautogui.press("volumeup")
        speak_resp("volume_up_fallback") or speak("Громкость увеличена.")
    except Exception:
        speak_resp("volume_up_fallback") or speak("Громкость увеличена.")
    return ""
...
def volume_down() -> str:
    ...
    try:
        ...
        speak_resp("volume_down_fallback") or speak("Громкость уменьшена.")
    except Exception:
        speak_resp("volume_down_fallback") or speak("Громкость уменьшена.")
    return ""
...
def brightness_up() -> str:
    try:
        ...
        key = f"brightness_{round(new/20)*20}"
        speak_resp(key) or speak(f"Яркость {new}%.")
        return ""
    except Exception:
        ...
        speak_resp("brightness_up"); return ""
...
def brightness_down() -> str:
    try:
        ...
        os.system("powershell (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods)"
                  ...
                  "-Class WmiMonitorBrightness).CurrentBrightness)-20))")
        speak_resp("brightness_down"); return ""

def wifi_toggle_on()  -> str:
    os.system('netsh interface set interface "Wi-Fi" enabled');  speak_resp("wifi_on"); return ""
def wifi_toggle_off() -> str:
    os.system('netsh interface set interface "Wi-Fi" disabled'); speak_resp("wifi_off"); return ""
def wifi_toggle()     -> str:
    if _wifi_status():
...
def screenshot() -> str:
    try:
        ...
        speak_resp("screenshot"); return ""
    except Exception: return None

def clipboard_copy()  -> str:
    import pyautogui; pyautogui.hotkey("ctrl","c"); speak_resp("clipboard_copy"); return ""
def clipboard_paste() -> str:
    import pyautogui; pyautogui.hotkey("ctrl","v"); speak_resp("clipboard_paste"); return ""
...
def switch_window()   -> str:
    import pyautogui; pyautogui.hotkey("alt","tab"); speak_resp("switch_window"); return ""
def window_minimize() -> str:
    import pyautogui; pyautogui.hotkey("win","d");   speak_resp("window_minimize"); return ""
def window_maximize() -> str:
    import pyautogui; pyautogui.hotkey("win","up");  speak_resp("window_maximize"); return ""
def window_close()    -> str:
    import pyautogui; pyautogui.hotkey("alt","f4");  speak_resp("window_close"); return ""
...
def open_app(name: str) -> str:
    ...
    app_key_map = {
        ...
        "word":"app_word","ворд":"app_word",
        "excel":"app_excel","эксель":"app_excel",
    }
    try:
...
def break_reminder_off() -> str:
    global break_reminder_active; break_reminder_active = False; speak_resp("break_off"); return ""
...
def break_code(): speak_resp("farewell") or speak("До встречи!"); time.sleep(1); exit()
...
def main():
    ...
    detector = WakeWordDetector(callback=_on_wake)
    detector.start()
