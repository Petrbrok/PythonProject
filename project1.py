...
class SlotExtractor:
    _WORDS = {
        ...
        "час":60, "полчаса":30, "четверть":15,
    }

    @classmethod
    def _replace_words(cls, text):
        for word, num in cls._WORDS.items():
    ...
    def time_to_seconds(text):
        ...
        if any(x in text for x in ["полчаса", "пол часа"]):
            return 1800
        text = SlotExtractor._replace_words(text)
        total = 0
...
def _process(query):
    ...
    # Зона 3: короткие запросы → unclear, длинные → ИИ
    if AI_ENABLED and _vosk_listener and len(query.split()) >= 3:
